...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const heroImages = document.querySelectorAll(".home-hero-images");
  ...
  const homeHeroTxt = document.querySelectorAll(".home-hero-text-wrap span h1");
  ...
  function fadeOutLoader() {
    tl.to(heroImages, { height: "30rem", duration: 2, ease: "expo.inOut" }); // "" to sync with loader fade-out
    tl.to(
      loader,
      {
        height: 0,
    ...
    tl.to(heroImage2, { x: "190%", duration: 2, ease: "expo.inOut" }, "h");
    tl.to(heroImage3, { x: "-190%", duration: 2, ease: "expo.inOut" }, "h");

    tl.to(".home-hero-images img", { opacity:0.5, duration: 2, ease: "expo.inOut",  },"h");
  }
...
cards.forEach((card) => {
  wwdTl.to(card.id, {
    ...
    rotate: card.rotate,
    ease: "power3.inOut",
  }, 0); // The '0' here aligns the animations to start at the same time.
});
...
document.querySelectorAll('.h-pro-image img, .h-pro-image-xl img').forEach((projectImg, index) => {
  gsap.to(projectImg, {
      y: -50,
      duration: 2,
      scrollTrigger: {
...
homeCGIBG.to(".home-cgi-videos-wrap", { backgroundColor: "#a3a3a3", duration: 1 });
    ...
    class Shuffle {
      ...
      shuffleText() {
          ...
          for (let i = 0; i < totalLetters; i++) {
              ...
              timeline.to(this.element, { duration: letterDuration, textContent: shuffledArray.join(''), ease: 'none' }, `+=${letterDuration}`);
          }
